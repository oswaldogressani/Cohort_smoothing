...
Cohort_Smoothing_No_Symmetry = function(m, n, Y, E, input.Pen, 	lambda1, lambda2, max.iter=10, max.iter.phi=20, dist="Poisson", fix.phi=FALSE, phi.input=phi.input){
		...
		if (dist=="Poisson"){
		  ...
		  if (ind.transformed.data==1){
		    for (it in 1:max.iter){ 
		    	mu <- Matrix(0, mmn, 1)
    			mu[input.M$pos] <- input.M$breve.e[input.M$pos] * exp(eta)[input.M$pos] * input.M$ww[input.M$pos]
  			  Psi <- Matrix(0, mmn,mmn)
			    Psi[cbind(input.M$pos, input.M$pos)] <- mu@x
			    pseudo <- Matrix(0, mmn, 1)
			    pseudo[input.M$pos] <- eta[input.M$pos] + 1/mu@x*(input.M$breve.y[input.M$pos] - mu@x)
  			  r <- Psi %*% pseudo
  			  WpP <- Psi + Pen
  			  etanew <- solve(WpP, r)
		      ...
		      eta.start <- eta				
		      eta.vec.tr.NS <- rep(NA,mmn)					# ETA VECtor on TRansformed data scale with No Symmetry
		...
		if (dist=="Neg.Bin2"){
		  ...
		  if (ind.transformed.data==1){
		    for (it1 in 1:max.iter.phi){
		      ...
		      mu.fit = exp(eta[input.M$pos]) * input.M$breve.e[input.M$pos]
		      y.fit = input.M$breve.y[input.M$pos]
		      f.d1 <- function (aa) { sum(((y.fit-mu.fit)^2)/(mu.fit*(1+aa*mu.fit))) - n*m + edf }
...
return(list(est.ETA.NS=ETA.NS, est.GAMMA.NS=GAMMA.NS, eta.start=eta.start, phi=phi))
}
...
Cohort_Smoothing_Symmetry = function(m, n, Y, E, p, 	input.Pen, 	start.values, lambda1, lambda2, 
	max.iter=10, 	max.iter.phi=20, dist="Poisson",	 	phi.input=0.00, 	get.V.matrix=FALSE, fix.phi=FALSE){
	...
	input.M = Create_Help_Data(m=m ,n=n, E=E, Y=Y)
		...
		if (ind.transformed.data==1){
			...
			# placing -1
			cc2a <- outer(seq(0, by=mm-1, length=n-1), seq(mm+m-1, length=m-1,by=mm), "+")
...
#################
		...
		if (dist=="Poisson"){
		  ...
		  if (ind.transformed.data==1){
		      ...
		      # check constraints
		      # boxplot(kappa - as.vector(L%*%eta.hat.vector))           # approx. zero
		  }
...
###################
		...
		if (dist=="Neg.Bin1"){
		  ...
		  if (ind.transformed.data==1){
		    ...
		    y.ll = input.M$breve.y[input.M$pos]
		    mu.ll = exp(eta[input.M$pos]) * input.M$breve.e[input.M$pos]
		    ll = -2*sum( lfactorial(y.ll + mu.ll/phi - 1) - lfactorial(y.ll+1-1) - lfactorial(mu.ll/phi - 1) + mu.ll/phi*log((mu.ll/phi)/(mu.ll/phi+mu.ll)) + y.ll*log((mu.ll)/(mu.ll/phi+mu.ll)) )
		  ...
		  if (ind.transformed.penalty==1){
		    ...
		    # Deviance
		    mu.ll = exp(eta) * e
		    ll = -2*sum( lfactorial(y + mu.ll/phi - 1) - lfactorial(y+1-1) - lfactorial(mu.ll/phi - 1) + mu.ll/phi*log((mu.ll/phi)/(mu.ll/phi+mu.ll)) + y*log((mu.ll)/(mu.ll/phi+mu.ll)) )
...
###################
		...
		if (dist=="Neg.Bin2"){
		  ...
		  if (ind.transformed.data==1){
		    ...
		    y.ll = input.M$breve.y[input.M$pos]
		    mu.ll = exp(eta[input.M$pos]) * input.M$breve.e[input.M$pos]
		    ll = -2*sum( lfactorial(y.ll + 1/phi - 1) - lfactorial(y.ll+1-1) - lfactorial(1/phi - 1) + 1/phi*log((1/phi)/(1/phi+mu.ll)) + y.ll*log((mu.ll)/(1/phi+mu.ll)) )
		  ...
		  if (ind.transformed.penalty==1){
		    ...
		    # Deviance
		    mu.ll = exp(eta) * e
		    ll = -2*sum( lfactorial(y + 1/phi - 1) - lfactorial(y+1-1) - lfactorial(1/phi - 1) + 1/phi*log((1/phi)/(1/phi+mu.ll)) + y*log((mu.ll)/(1/phi+mu.ll)) )
...
return(list(est.ETA.S=ETA.S, est.GAMMA.S=GAMMA.S, conv.ind=conv.ind, edf=edf, ll=ll, aic=aic, bic=bic, V=V, phi=phi))
}
