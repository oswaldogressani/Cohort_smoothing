...
image.plot(plot.x1, plot.x2, GAMMA.true, col=topo.colors(19),
           zlim=c(0.00, 0.85),
...
plot.x1 = (lower_index:upper_index) - 1
plot.x2 = (lower_index:upper_index) - 1
image.plot(plot.x1, plot.x2, log(GAMMA.true), col=topo.colors(19),
           zlim=c(-4.20, 0.00),
           xlab="Age of the respondent", ylab="Age of the contact",
           cex.lab=1.6, cex.axis=1.6)
...
pen_method2_kink = Penalty_Transformed_Dataset(m = m, n = n, kink = TRUE, 
                                               max.kink.age = 30)
pen_method3_no_kink = Penalty_Transformed_Penalty(m = m, n = n, kink = FALSE)
pen_method3_kink = Penalty_Transformed_Penalty(m = m, n = n, kink = TRUE,
                                               max.kink.age = 30)
...
estimated.ETAmatrix1 = estimated.ETAmatrix2 = estimated.ETAmatrix3 = 
  estimated.ETAmatrix4 = estimated.ETAmatrix5 = list()
estimated.COVmatrix1 = estimated.COVmatrix2 = estimated.COVmatrix3 =
  estimated.COVmatrix4 = estimated.COVmatrix5 = list()
...
for (k in 1:sim.N){
  set.seed(seed.in + k)
	...
	### grid search
	# grid.l1 = grid.l2 = c(100, 150, 200, 250, 300)
	# grid.l1 = c(50,100,200,300,500)
	# grid.l2 = c(500,1000,2000,3000,4000)
	# grid.search = Grid_Search_No_Cohort_Smoothing(m=m, n=n, Y=Y, E=E, p=p,
	# grid.l1=grid.l1, grid.l2=grid.l2, dist="Poisson")
	# grid.search = Grid_Search_Cohort_Smoothing(m=m, n=n, Y=Y, E=E, p=p, 
	# grid.l1=grid.l1, grid.l2=grid.l2, dist="Poisson", PEN=pen_method3_kink)
	# z = t(matrix(grid.search$out[,5],5,5))
	# range(z)

	## Analsis with no smoothing over cohorts - Poisson distribution
	fit0.poisson.method1 = No_Cohort_Smoothing_No_Symmetry(m=m, n=n, Y=Y, E=E, 
	                       lambda1 = 200, lambda2 = 200, max.iter = 25, dist="Poisson")
	fit1.poisson.method1 = No_Cohort_Smoothing_Symmetry(m=m, n=n, Y=Y, E=E, p=p, 
	                                                    lambda1=200, lambda2=200, start.values=fit0.poisson.method1$eta.start, 
	...
	fit1.poisson.method3 = Cohort_Smoothing_Symmetry(m=m, n=n, Y=Y, E=E, p=p, input.Pen=pen_method3_no_kink,
	                                                 lambda1=100, lambda2=1000, start.values=fit0.poisson.method3$eta.start, 
	                                                 max.iter=100, get.V.matrix=TRUE, dist="Poisson")

	## Analsis with transformed data set - Poisson distribution - with kink
	fit1.poisson.method4 = Cohort_Smoothing_Symmetry(m=m, n=n, Y=Y, E=E, p=p, input.Pen=pen_method2_kink, 
	                                                 lambda1=100, lambda2=1000, start.values=fit0.poisson.method2$eta.start, 
	                                                 max.iter=100, get.V.matrix=TRUE, dist="Poisson")
	
	## Analsis with transformed penalty - Poisson distribution  - with kink
	fit1.poisson.method5 = Cohort_Smoothing_Symmetry(m=m, n=n, Y=Y, E=E, p=p, input.Pen=pen_method3_kink,
	                                                 lambda1=100, lambda2=1000, start.values=fit0.poisson.method3$eta.start, 
	                                                 max.iter=100, get.V.matrix=TRUE, dist="Poisson")
...
save(matrix.conv.ind1, matrix.conv.ind2, matrix.edf, matrix.ll, matrix.aic, matrix.bic, matrix.phi,
     ...
     file="newoutput/Poisson_no_kink.RData")
...
load("newoutput/Poisson_no_kink.RData")
...
image.plot(x1-1,x2-1,ETA.TRUE,zlim=c(-4.2,0.0),col=topo.colors(20), main="True surface", 
...
par(mfrow=c(1,2))
par(mar=c(5.1,5.1,2.1,1.5))
plot(diag(ETA.TRUE), xlab="Index main diagonal element", ylab="Log contact rate", cex.axis=1.6, cex.lab=1.4, ylim=c(-2.2, 0.0))
...
par(mfrow=c(3,2))
par(mar=c(5.1,5.1,2.1,1.5))
image.plot(x1-1,x2-1,GAMMA.TRUE,zlim=c(0.0,0.85),col=topo.colors(20), main="True surface", 
           xlab="Age of the respondent", ylab="Age of the contact", cex.lab=1.6, cex.axis=1.6)
image.plot(x1-1,x2-1,bias.GAMMA1,zlim=c(-0.13,0.13),col=topo.colors(20), main="Model M0 without kink",
           xlab="Age of the respondent", ylab="Age of the contact", cex.lab=1.6, cex.axis=1.6)
image.plot(x1-1,x2-1,bias.GAMMA2,zlim=c(-0.13,0.13),col=topo.colors(20), main="Model M1 without kink",
           xlab="Age of the respondent", ylab="Age of the contact", cex.lab=1.6, cex.axis=1.6)
image.plot(x1-1,x2-1,bias.GAMMA3,zlim=c(-0.13,0.13),col=topo.colors(20), main="Model M2 without kink",
           xlab="Age of the respondent", ylab="Age of the contact", cex.lab=1.6, cex.axis=1.6)
image.plot(x1-1,x2-1,bias.GAMMA4,zlim=c(-0.13,0.13),col=topo.colors(20), main="Model M1 with kink",
           xlab="Age of the respondent", ylab="Age of the contact", cex.lab=1.6, cex.axis=1.6)
image.plot(x1-1,x2-1,bias.GAMMA5,zlim=c(-0.13,0.13),col=topo.colors(20), main="Model M2 with kink",
           xlab="Age of the respondent", ylab="Age of the contact", cex.lab=1.6, cex.axis=1.6)
...
par(mfrow=c(1,2))
par(mar=c(5.1,5.1,2.1,1.5))
plot(diag(GAMMA.TRUE), xlab="Index main diagonal element", ylab="Contact rate", cex.axis=1.6, cex.lab=1.4, ylim=c(0.0, 0.90))
lines(diag.GAMMA1, col=1, lwd=2.8)
lines(diag.GAMMA2, col=2, lwd=2.8)
lines(diag.GAMMA3, col=3, lwd=2.8, lty=2)
legend(30, 0.92, c("M0 w/o kink","M1 w/o kink","M2 w/o kink"), col=1:3, lty=c(1,1,2), lwd=2.8, bty="n")

plot(diag(GAMMA.TRUE), xlab="Index main diagonal element", ylab="Contact rate", cex.axis=1.6, cex.lab=1.4, ylim=c(0.0, 0.90))
lines(diag.GAMMA2, col=2, lwd=2.8)
lines(diag.GAMMA3, col=3, lwd=2.8, lty=2)
legend(30, 0.92, c("M1 w kink","M2 w kink"), col=2:3, lty=c(1,2), lwd=2.8, bty="n")
...
image.plot(x1-1,x2-1,mse.ETA3,zlim=c(0.00,0.22),col=topo.colors(20), 
           xlab="Age of the respondent", ylab="Age of the contact", cex.lab=1.6, cex.axis=1.6)
...
range(mse.GAMMA3)
par(mar=c(5.1,6.1,2.1,1.5))
image.plot(x1-1, x2-1, mse.GAMMA3, zlim=c(0.00,0.005), col=topo.colors(20), 
           xlab="Age of the respondent", ylab="Age of the contact", cex.lab=1.6, cex.axis=1.6)

## Create Table 1
simpoiss_nokink <- matrix(0, nrow = 2, ncol = 10)
rownames(simpoiss_nokink) <- c("Poiss-no-kink-bias^2","Poiss-no-kink-MSE")
colnames(simpoiss_nokink) <-c("M0","M1","M2","M0","M1","M2","M1","M2","M1","M2")
## Enter the bias
simpoiss_nokink[1,1:3] <- c(sum(bias.ETA1^2), sum(bias.ETA2^2), sum(bias.ETA3^2))
simpoiss_nokink[1,4:6] <- c(sum(bias.GAMMA1^2), sum(bias.GAMMA2^2), sum(bias.GAMMA3^2))
simpoiss_nokink[1,7:8] <- c(sum(bias.ETA4^2), sum(bias.ETA5^2))
simpoiss_nokink[1,9:10] <- c(sum(bias.GAMMA4^2),sum(bias.GAMMA5^2))
## Enter the MSE
simpoiss_nokink[2,1:3] <- c(sum(mse.ETA1), sum(mse.ETA2), sum(mse.ETA3))
simpoiss_nokink[2,4:6] <- c(sum(mse.GAMMA1), sum(mse.GAMMA2), sum(mse.GAMMA3))
simpoiss_nokink[2,7:8] <- c(sum(mse.ETA4), sum(mse.ETA5))
simpoiss_nokink[2,9:10] <- c(sum(mse.GAMMA4),sum(mse.GAMMA5))

# Coverage results
simpoiss_nokink_coverage <- matrix(0, nrow = 1, ncol = 5)
colnames(simpoiss_nokink_coverage) <- c("M0","M1","M2","M1","M2")
rownames(simpoiss_nokink_coverage) <- "Poiss-no-kink-coverage"
...
simpoiss_nokink_coverage[1,] <- round(c(mean(cov1)*100,
                                  mean(cov2)*100,
                                  mean(cov3)*100,
                                  mean(cov4)*100,
                                  mean(cov5)*100),2)
...
round(simpoiss_nokink,2)
simpoiss_nokink_coverage
